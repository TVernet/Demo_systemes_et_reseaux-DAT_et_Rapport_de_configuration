...
Un durcissement des règles `sysctl` est détaillé dans la partie "Rapport de Configuration", section "3.1". Elle comprend l'activation des Privacy Extensions IPv6, le blocage du Forwarding entre les protocoles IPv4 et IPv6 et une protection contre les SYN flood ; +

Pour la configuration Netfilter, La solution de filtrage retenue est l'outil `Iptables`. Sa configuration complète est détaillée dans la partie "Rapport de Configuration", section "3.2". +

Le résumé des mesures prises pour le filtrage IPv4 et IPv6 ; +
